...
- **[PostgreSQL State Backend](postgres-backend.md)** — Optional Postgres-backed session and state storage in place of the default single-file SQLite database, for multi-host deployments or filesystems where SQLite is unsuitable. Includes a source-safe one-shot migration script.
